...
namespace melonDS
{
extern const char* k2DArtCNNPassVS;
extern const char* k2DArtCNN_RGBToYUVAFS;
extern const char* k2DArtCNN_Spline36FS;
extern const char* k2DArtCNN_YUVAToRGBA2xFS;
extern const char* k2DArtCNN_C4F16_Conv0FS;
extern const char* k2DArtCNN_C4F16_Conv1FS;
extern const char* k2DArtCNN_C4F16_Conv2FS;
extern const char* k2DArtCNN_C4F16_Conv3FS;
extern const char* k2DArtCNN_C4F16_Conv4FS;
extern const char* k2DArtCNN_C4F16_Conv5FS;
extern const char* k2DArtCNN_C4F16_Conv6FS;
extern const char* k2DArtCNN_C4F16_DepthToSpaceFS;
extern const char* k2DArtCNN_C4F16DN_Conv0FS;
extern const char* k2DArtCNN_C4F16DN_Conv1FS;
extern const char* k2DArtCNN_C4F16DN_Conv2FS;
extern const char* k2DArtCNN_C4F16DN_Conv3FS;
extern const char* k2DArtCNN_C4F16DN_Conv4FS;
extern const char* k2DArtCNN_C4F16DN_Conv5FS;
extern const char* k2DArtCNN_C4F16DN_Conv6FS;
extern const char* k2DArtCNN_C4F16DN_DepthToSpaceFS;
extern const char* k2DNNEDI3_CommonFS;
extern const char* k2DNNEDI3_NNS16Win8x4KernelFS;
...
bool TexcacheOpenGLLoader::EnsureArtCNNPrograms()
{
    ...
    if (!OpenGL::CompileVertexFragmentProgram(ArtCNNRGBToYUVAShader,
            ...
            "3DArtCNNRGBToYUVAShader",
            {{"vPosition", 0}},
            {{"oColor", 0}}))
        goto fail;

    if (!OpenGL::CompileVertexFragmentProgram(ArtCNNRepackShader,
            k2DArtCNNPassVS, BuildTexcacheRepackShaderSource(false),
    ...
    if (!OpenGL::CompileVertexFragmentProgram(ArtCNNRepackFilterableShader,
            k2DArtCNNPassVS, BuildTexcacheRepackShaderSource(true),
    ...
    glUniform1i(glGetUniformLocation(ArtCNNRGBToYUVAShader, "Source"), 0);
    ...
    for (int model = 0; model < RendererSettings::GLArtCNNModelCount; model++)
    {
        ...
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][0], "artcnn_yuv"), 0);

        glUseProgram(ArtCNNConvShaders[model][1]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][1], "conv2d"), 0);

        glUseProgram(ArtCNNConvShaders[model][2]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][2], "conv2d_1"), 0);

        glUseProgram(ArtCNNConvShaders[model][3]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][3], "conv2d_2"), 0);

        glUseProgram(ArtCNNConvShaders[model][4]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][4], "conv2d_3"), 0);

        glUseProgram(ArtCNNConvShaders[model][5]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][5], "conv2d_4"), 0);

        glUseProgram(ArtCNNConvShaders[model][6]);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][6], "conv2d"), 0);
        glUniform1i(glGetUniformLocation(ArtCNNConvShaders[model][6], "conv2d_5"), 1);

        glUseProgram(ArtCNNDepthToSpaceShaders[model]);
        glUniform1i(glGetUniformLocation(ArtCNNDepthToSpaceShaders[model], "conv2d_6"), 0);
    }

    glUseProgram(ArtCNNSpline36Shader);
    glUniform1i(glGetUniformLocation(ArtCNNSpline36Shader, "Source"), 0);
    ...
    glUniform1i(glGetUniformLocation(NNEDI3Pass1Shader, "Source"), 0);

    glUseProgram(NNEDI3Pass2Shader);
    glUniform1i(glGetUniformLocation(NNEDI3Pass2Shader, "Source"), 0);
...
bool TexcacheOpenGLLoader::EnsureArtCNNBuffers()
{
    ...
    if (ArtCNNPassVBO == 0)
    {
        ...
        glGenBuffers(1, &ArtCNNPassVBO);
        glBindBuffer(GL_ARRAY_BUFFER, ArtCNNPassVBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
    }

    if (ArtCNNPassVAO == 0)
    {
        glGenVertexArrays(1, &ArtCNNPassVAO);
        glBindVertexArray(ArtCNNPassVAO);
        glBindBuffer(GL_ARRAY_BUFFER, ArtCNNPassVBO);
...
bool TexcacheOpenGLLoader::EnsureArtCNNTextures(u32 width, u32 height, u32 scaleFactor)
{
    ...
    DeleteTextureIfNeeded(NNEDI3Luma4xCorrectedTex);
    DeleteTextureIfNeeded(ArtCNNYUVA2xTex);
    DeleteTextureIfNeeded(NNEDI3YUVA4xTex);
    DeleteTextureIfNeeded(ArtCNNRGBA2xTex);
    DeleteTextureIfNeeded(NNEDI3RGBA4xTex);
    ...
    NNEDI3Luma4xCorrectedTex = CreateArtCNNTexture(GL_RGBA16F, width4x, height4x);
    ArtCNNYUVA2xTex = CreateArtCNNTexture(GL_RGBA16F, width2x, height2x);
    NNEDI3YUVA4xTex = CreateArtCNNTexture(GL_RGBA16F, width4x, height4x);
    ArtCNNRGBA2xTex = CreateArtCNNTexture(GL_RGBA8, width2x, height2x);
    NNEDI3RGBA4xTex = CreateArtCNNTexture(GL_RGBA8, width4x, height4x);
...
void TexcacheOpenGLLoader::DeleteArtCNNResources()
{
    ...
    if (ArtCNNPassVAO != 0)
    {
        glDeleteVertexArrays(1, &ArtCNNPassVAO);
        ArtCNNPassVAO = 0;
    }
    if (ArtCNNPassVBO != 0)
    {
        glDeleteBuffers(1, &ArtCNNPassVBO);
        ArtCNNPassVBO = 0;
    }

    std::array<GLuint*, 6 + RendererSettings::GLArtCNNModelCount * 8> shaders = {
        &ArtCNNRGBToYUVAShader,
...
void TexcacheOpenGLLoader::RenderArtCNNPass(GLuint shader, GLuint outputTex, int width, int height, GLuint source0, GLuint source1)
{
...
void TexcacheOpenGLLoader::RenderArtCNNSpline36(GLuint sourceTex, GLuint targetTex, int width, int height, float sourceShiftX, float sourceShiftY)
{
    glUseProgram(ArtCNNSpline36Shader);
    ...
    RenderArtCNNPass(ArtCNNSpline36Shader, targetTex, width, height, sourceTex, 0);
}

void TexcacheOpenGLLoader::RenderXBRZ(GLuint sourceTex, GLuint targetTex, int width, int height, u32 scaleFactor)
{
    RenderArtCNNPass(XBRZPreprocessShader, XBRZInfoTex, width, height, sourceTex, 0);
    RenderArtCNNPass(XBRZFreescaleShader, targetTex, width * (int)scaleFactor, height * (int)scaleFactor, sourceTex, XBRZInfoTex);
}
...
bool TexcacheOpenGLLoader::ProcessTextureGPUScaleToTexture(u32 width, u32 height, u32 scaleFactor, const u32* sourceRGBA, GLuint& outputTexture)
{
    ...
    {
        RenderArtCNNSpline36(ArtCNNSourceTex, ArtCNNOutputTex, width * scaleFactor, height * scaleFactor, 0.0f, 0.0f);
    }
    ...
    {
        RenderArtCNNPass(ArtCNNRGBToYUVAShader, ArtCNNYUVTex, width, height, ArtCNNSourceTex, 0);

        if (RendererSettings::IsGLNNEDI3Algorithm(ScaleAlgorithm))
        {
            RenderArtCNNPass(NNEDI3Pass1Shader, NNEDI3VerticalTex, width, height * 2, ArtCNNYUVTex, 0);
            RenderArtCNNPass(NNEDI3Pass2Shader, ArtCNNLuma2xTex, width * 2, height * 2, NNEDI3VerticalTex, 0);
            if (scaleFactor >= 4)
            {
                RenderArtCNNPass(NNEDI3Pass1Shader, NNEDI3Vertical4xTex, width * 2, height * 4, ArtCNNLuma2xTex, 0);
                RenderArtCNNPass(NNEDI3Pass2Shader, NNEDI3Luma4xTex, width * 4, height * 4, NNEDI3Vertical4xTex, 0);
                RenderArtCNNSpline36(ArtCNNYUVTex, NNEDI3YUVA4xTex, width * 4, height * 4, 0.0f, 0.0f);
                RenderArtCNNSpline36(NNEDI3Luma4xTex, NNEDI3Luma4xCorrectedTex, width * 4, height * 4, -1.5f, -1.5f);

                GLuint rgba4xTarget = (scaleFactor == 4) ? ArtCNNOutputTex : NNEDI3RGBA4xTex;
                RenderArtCNNPass(ArtCNNYUVAToRGBA2xShader, rgba4xTarget, width * 4, height * 4, NNEDI3YUVA4xTex, NNEDI3Luma4xCorrectedTex);

                if (scaleFactor > 4)
                    RenderArtCNNSpline36(NNEDI3RGBA4xTex, ArtCNNOutputTex, width * scaleFactor, height * scaleFactor, 0.0f, 0.0f);
            }
            else
            {
                RenderArtCNNSpline36(ArtCNNLuma2xTex, ArtCNNConvWorkTex[0], width * 2, height * 2, -0.5f, -0.5f);
                RenderArtCNNSpline36(ArtCNNYUVTex, ArtCNNYUVA2xTex, width * 2, height * 2, 0.0f, 0.0f);

                GLuint rgba2xTarget = (scaleFactor == 2) ? ArtCNNOutputTex : ArtCNNRGBA2xTex;
                RenderArtCNNPass(ArtCNNYUVAToRGBA2xShader, rgba2xTarget, width * 2, height * 2, ArtCNNYUVA2xTex, ArtCNNConvWorkTex[0]);

                if (scaleFactor > 2)
                    RenderArtCNNSpline36(ArtCNNRGBA2xTex, ArtCNNOutputTex, width * scaleFactor, height * scaleFactor, 0.0f, 0.0f);
            }
        }
        else
        {
            ...
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][0], ArtCNNConv0Tex, width * 2, height * 2, ArtCNNYUVTex, 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][1], ArtCNNConvWorkTex[0], width * 2, height * 2, ArtCNNConv0Tex, 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][2], ArtCNNConvWorkTex[1], width * 2, height * 2, ArtCNNConvWorkTex[0], 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][3], ArtCNNConvWorkTex[0], width * 2, height * 2, ArtCNNConvWorkTex[1], 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][4], ArtCNNConvWorkTex[1], width * 2, height * 2, ArtCNNConvWorkTex[0], 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][5], ArtCNNConvWorkTex[0], width * 2, height * 2, ArtCNNConvWorkTex[1], 0);
            RenderArtCNNPass(ArtCNNConvShaders[modelIndex][6], ArtCNNPackedTex, width, height, ArtCNNConv0Tex, ArtCNNConvWorkTex[0]);
            RenderArtCNNPass(ArtCNNDepthToSpaceShaders[modelIndex], ArtCNNLuma2xTex, width * 2, height * 2, ArtCNNPackedTex, 0);

            GLuint rgba2xTarget = (scaleFactor == 2) ? ArtCNNOutputTex : ArtCNNRGBA2xTex;
            RenderArtCNNPass(ArtCNNYUVAToRGBA2xShader, rgba2xTarget, width * 2, height * 2, ArtCNNYUVA2xTex, ArtCNNLuma2xTex);
                                                          ...
                                                          int width, int height, int outputFmt, bool binaryAlpha)
{
    glBindFramebuffer(GL_READ_FRAMEBUFFER, 0);
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, ArtCNNOutputFB);
    glFramebufferTextureLayer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, targetArrayTexture, 0, (GLint)targetLayer);
    glDrawBuffer(GL_COLOR_ATTACHMENT0);
    ...
    if (FilterableSampling)
    {
        glBindTexture(GL_TEXTURE_2D_ARRAY, targetArrayTexture);
        glGenerateMipmap(GL_TEXTURE_2D_ARRAY);
    }
}
                                                              ...
                                                              std::vector<u32>* outputPreviewRGBA)
{
    ...
    RenderArtCNNRepackToArrayLayer(outputTexture, targetArrayTexture, targetLayer,
